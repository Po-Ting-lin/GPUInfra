...
bool Mi::configure(const AlgoRuntimeInfo& info) {
...
bool Mi::allocateOutputBuffers(const ThreadSlot& slot) {
...
bool Mi::launchKernels(const ThreadSlot& slot, cudaStream_t stream) {
    ...
    miMatrixMultiplicationKernel << <grid, block, 0, stream >> > (d_input, impl->d_outputMatrix, impl->matrixSize, impl->inputStride);
