...
bool Cel::init(const AlgoRuntimeInfo& info, const TaskGpuResources& resources, std::size_t& scratchBytes) {
    scratchBytes = 0;
    if (resources.resourceId < 0 || resources.gpuId < 0 || resources.stream == nullptr || !ImageSizing::isValidFactor(info.sizeFactor)) {
        return false;
    }
    const int outputSize = ImageSizing::scaledDimension(info.sizeFactor, ImageSizing::CEL_MULTIPLIER);
...
bool Cel::notifyParameter(const AlgoParams& params) {
    if (!initialized) {
        return false;
    }
...
bool Cel::launchKernels(const TaskGpuResources& resources, const void* d_frameData, cudaStream_t stream) {
    ...
    celMatrixMultiplicationKernel << <grid, block, 0, stream >> > (d_input, d_outputMatrix, matrixSize, frameW);
    CUDA_CHECK(cudaGetLastError(), return false);
    return true;
}

bool Cel::launchD2H(const TaskGpuResources& resources, cudaStream_t stream) {
...
bool Cel::collectResult(const TaskGpuResources& resources, AlgoOutput& output) const {
    if (!initialized || resources.resourceId != resourceId || resources.gpuId != gpuId) {
        return false;
    }

    if (output.algoName != "cel" || output.width != matrixSize || output.height != matrixSize || output.data.size() != matrixBytes) {
        return false;
    }
